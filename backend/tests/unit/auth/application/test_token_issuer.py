...
def test_issue_refresh_token_stores_digest_and_propagates_metadata(
    ...
    plaintext, token = token_issuer.issue_refresh_token(user_id=user_id, client_metadata=metadata)

    assert token.token_hash == deterministic_hasher.digest(secret=plaintext)
    assert token.user_id == user_id
    assert token.ip_address == "10.0.0.1"
    assert token.user_agent == "UA/1.0"
    assert token.expires_at > dt.datetime.now(tz=dt.UTC)
...
def test_build_token_pair_access_token_decodes_back_to_user(
    ...
    refresh_secret, refresh_token = token_issuer.issue_refresh_token(
    ...
    assert jwt_service.decode_access_token(pair.access_token.get_secret_value()) == credentials.user_id
    assert pair.refresh_token.secret.get_secret_value() == refresh_secret
    assert pair.refresh_token.expires_at == refresh_token.expires_at
