...
class CityNames:
    """
    Имена города по языкам (value object).

    Группирует разноязычные написания города в один тип, чтобы они не болтались
    отдельными полями на ``City``. Резолв имени — единый параметризованный (``get``
