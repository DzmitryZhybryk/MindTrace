...
- **Приветствие на дашборде стало живым** — имя из нового `/v1/users/me` (`displayName`, фоллбэк на `username`; email в фоллбэке не участвует), дата — реальная текущая в языке интерфейса вместо захардкоженных «Dzmitry» и «Saturday · May 2026». Блок появляется целиком после загрузки профиля (без вспышки «Hello,» без имени), при ошибке загрузки — только дата; инициалы аватарки в шапке — тоже из профиля
