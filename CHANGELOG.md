...
- **Разовый сбой сети больше не оставляет экран пустым навсегда.** Server-state переехал на TanStack Query: профиль (`/v1/users/me`), карта поездок и автокомплит городов повторяют запрос с экспоненциальным backoff'ом на транзиентных сбоях (сеть, 5xx) и не повторяют 4xx — ответ «нет прав»/«не найдено»/«удалён» приходит пользователю сразу, а не после трёх бессмысленных попыток
- **Весь HTTP-слой фронта — сгенерированный из OpenAPI SDK.** Рукописные `api/auth.ts`, `api/users.ts`, `api/journeys.ts` удалены; ответы валидируются zod-схемами из той же схемы, что отдаёт бэк, поэтому расхождение контракта ловится в браузере как машинный код `invalid_response`, а не как молчаливый `undefined` в середине рендера. Сессия (Bearer, refresh-cookie, single-flight refresh, повтор запроса после обновления токена) осталась в собственном транспорте, поверх которого работает SDK
- **Карта поездок и глобус-фон читают один кэш.** Добавленная поездка обновляет оба сразу — раньше глобус показывал прежние города до перезагрузки страницы. Возврат на дашборд больше не перезапрашивает карту: фон живёт со снимка, а обновляется по факту изменения данных
- **Данные ушедшей сессии не достаются следующему входу** — кэш чистится на смене состояния сессии, включая недобровольный разлогин (протухший refresh, удалённый аккаунт), а не только по кнопке «Выйти»
- Цена по весу — +9.6 kb gz на каждую страницу (лендинг 191 kb при бюджете 200, карта поездок 296 при 300); рукописный кэш городов на 111 строк при этом удалён

### Backend 1.1.1

- **OpenAPI-схема подготовлена под кодоген фронтового SDK**: `operationId` собирается из имени обработчика (устойчивые имена операций вместо автогенерируемых с путём и методом), теги без точки, ответ 422 описан как `ErrorResponse` — то есть как сервис его и отдаёт (раньше в схеме стоял дефолтный `HTTPValidationError` от FastAPI, а в рантайме приходил единый конверт ошибки). Поведение эндпоинтов не менялось
- Схема закоммичена в `backend/openapi.json` и защищена снапшот-тестом в `make check`: расхождение схемы приложения с файлом роняет гейт до пуша
...
- **`current_user_id_dependency` и `InvalidAccessTokenError` переехали из `auth/presentation` в shared jwt-вертикаль** (`app/shared/infra/jwt/`) — аутентификация запроса это cross-cutting инфраструктура, а не логика домена auth. Presentation-слои geo/journeys/users больше не зависят от `auth.presentation`, взаимный цикл auth ↔ users разомкнут, в tach включён `forbid_circular_dependencies` (возврат любого цикла роняет гейт). Код ошибки `auth.invalid_access_token` не менялся — внешний контракт
...
- **Приветствие на дашборде стало живым** — имя из нового `/v1/users/me` (`displayName`, фоллбэк на `username`; email в фоллбэке не участвует), дата — реальная текущая в языке интерфейса вместо захардкоженных «Dzmitry» и «Saturday · May 2026». Дата видна сразу (от сети она не зависит), строка приветствия появляется после загрузки профиля на заранее зарезервированном месте — без вспышки «Hello,» без имени и без сдвига вёрстки; инициалы аватарки в шапке — тоже из профиля. Коды `users.user_deleted`/`users.user_not_found` в ответе `/me` завершают сессию (разлогин)

## 2026-08-14

### Project

- **Coverage-гейт перенесён с pre-push на pre-commit** — порог 90% обеих сторон теперь проверяется на каждом commit (`.githooks/pre-commit`), а не при push; обход в исключительном случае — `git commit --no-verify`

### Frontend 2.3.0

- **Подписи городов на глобусе больше не наезжают друг на друга** — деклаттер в экранных координатах: у пересёкшихся подписей плавно гаснет текст менее приоритетной (ближе к горизонту, где он и так сжат проекцией) и возвращается, когда вращение освобождает место; возврат — с запасом зазора против мерцания. Точка города видима всегда, прячется только текст

### Frontend 2.2.0

- **Глобус на дашборде вращается курсором и пальцем** — обеими осями, так что южное полушарие (при дефолтной камере навсегда «под подбородком» планеты) теперь досягаемо. Хит-тест жеста пиксельный, по самой сфере (`toGlobeCoords`-raycast), а не по прямоугольнику канваса: клики и скролл мимо шара работают как раньше, на тач-раскладке страница скроллится с любого места вне сферы. Во время драга автовращение ставится на паузу и возобновляется с нового места; на остальных гранях (лендинг, login/signup) глобус остаётся чисто декоративным
...
- **Advisories больше не сюрприз в фичевом PR.** Audit-шаги гейтов ходят в живую базу — проходивший гейт краснеет сам, когда публикуют advisory (четвёртый случай: click, gitpython ×2, undici/postcss). Два контура на замену ручному циклу: **ночной `audit.yml`** (schedule 03:00 UTC + workflow_dispatch) гоняет те же `make audit` обеих сторон по `dev` — advisory приходит утренним письмом, а не красным PR; **Dependabot-экосистема `uv`** для `backend/uv.lock` (weekly, minor/patch одной группой — симметрично npm) — закрыта слепая зона, из-за которой транзитивный gitpython бампился только руками
